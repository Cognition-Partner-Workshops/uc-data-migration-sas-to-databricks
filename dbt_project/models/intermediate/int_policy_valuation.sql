/*
  ...
  SAS Original:
    Step 1: PROC SQL extracting in-force policies with derived metrics
            (policy age, months to expiry, renewal flag, earned premium)
    Step 2: PROC SQL aggregating 12-month claims experience by policy
    Step 3: PROC SQL aggregating premium collections by policy
    Step 4: DATA step MERGE BY POLICY_ID assembling valuation metrics
            (loss ratio, combined ratio, premium adequacy, IBNR, reserves)

  dbt Equivalent:
    PROC SQL extracts → CTEs with SQL date functions
    SAS intck/intnx → months_between/date_add
    DATA step MERGE BY → SQL LEFT JOINs
    SAS coalesce/max/min → SQL equivalents
    SAS FORMAT $POLTYPE/$RISKCAT → dbt macros
*/
...
-- SAS Step 1: In-force policy extract with derived fields
inforce as (
    select
        p.policy_id,
        p.customer_id,
        ...
        -- SAS: Renewal due flag (within 3 months of expiry)
        case
            when p.expiration_date <= date_add(current_date(), 90)
            then 'Y' else 'N'
...
claims_exp as (
    ...
    from claims c
    where c.loss_date >= date_add(current_date(), -365)
...
-- SAS Step 4: MERGE BY POLICY_ID + valuation metric derivation
valuation as (
    select
        i.*,
        ce.num_claims,
        ...
        -- SAS: Combined Ratio = LOSS_RATIO + 0.30 (simplified expense load)
        case
            when i.ytd_earned_premium > 0
