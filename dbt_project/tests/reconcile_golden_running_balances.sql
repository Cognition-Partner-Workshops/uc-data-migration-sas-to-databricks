/*
  Golden parity: CURATED.RUNNING_BALANCES  <->  mart_running_balances

  Row-by-row comparison of the migrated model against the SAS golden output
  (seed sas_golden_running_balances, business date 31JAN2024). Keys: transaction_id.
  Exact columns: account_id, transaction_date.
  Tolerance columns: running_balance (macros/approx_equal.sql).
...
with m as (
    select * from {{ ref('mart_running_balances') }}
),

g as (
    select * from {{ ref('sas_golden_running_balances') }}
),
